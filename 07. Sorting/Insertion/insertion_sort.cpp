...
void insertion_sort(std::vector<int>& vec){
    for(int i = 0; i < vec.size() - 1; i++){
        ...
        for(int j = i + 1; j < vec.size(); j++){
            if(vec.at(j) < vec.at(key)){
                key = j;
            }
        }
        std::swap(vec.at(i), vec.at(key));
    }
}

int main(){
    const int MAX = 1000;
    std::vector<int> v_ran(1000000);
    std::vector<int> v_in(1000000);
    std::vector<int> v_dec(1000000);
